...
$return_array = _yani_agent()->render_property_contact_form();;
if(empty($return_array)) {
	return;
}
$terms_page_id = _yani_theme()->get_option('terms_condition');
$hide_form_fields = _yani_theme()->get_option('hide_prop_contact_form_fields');
$agent_display = _yani_property_listing()->get_listing_data('agent_display_option');
$property_id = _yani_property_listing()->get_listing_data('property_id');
...
$agent_mobile_num = _yani_theme()->get_option('agent_mobile_num', 1 ); 
$agent_whatsapp_num = _yani_theme()->get_option('agent_whatsapp_num', 1);
...
<div class="property-form-wrap">
	...
	if(_yani_form()->get_form_type()) {
		...
		if(!empty(_yani_theme()->get_option('contact_form_agent_above_image'))) {
			echo do_shortcode(_yani_theme()->get_option('contact_form_agent_above_image'));
		}
		...
		<div class="property-form clearfix">
			<form method="post" action="#">
				...
				<div class="form-group">
					<input class="form-control" name="name" value="<?php echo esc_attr($user_name); ?>" type="text" placeholder="<?php echo _yani_theme()->get_option('spl_con_name', 'Name'); ?>">
				</div><!-- form-group -->
				<?php } ?>

				<?php if( $hide_form_fields['phone'] != 1 ) { ?>	
				<div class="form-group">
					<input class="form-control" name="mobile" value="" type="text" placeholder="<?php echo _yani_theme()->get_option('spl_con_phone', 'Phone'); ?>">
				</div><!-- form-group -->
				<?php } ?>

				<div class="form-group">
					<input class="form-control" name="email" value="<?php echo esc_attr($user_email); ?>" type="email" placeholder="<?php echo _yani_theme()->get_option('spl_con_email', 'Email'); ?>">
				</div><!-- form-group -->

				<?php if( $hide_form_fields['message'] != 1 ) { ?>	
				<div class="form-group form-group-textarea">
					<textarea class="form-control hz-form-message" name="message" rows="4" placeholder="<?php echo _yani_theme()->get_option('spl_con_message', 'Message'); ?>"><?php echo _yani_theme()->get_option('spl_con_interested', "Hello, I am interested in"); ?> [<?php echo get_the_title(); ?>]</textarea>
				</div><!-- form-group -->	
				<?php } ?>
				...
				<button type="button" class="yani_agent_property_form btn btn-secondary <?php echo esc_attr($send_btn_class); ?>">
					<?php get_template_part('template-parts/loader'); ?>
					<?php echo _yani_theme()->get_option('spl_btn_send', 'Send Email'); ?>
				...
				<a href="tel:<?php echo esc_attr($agent_mobile_call); ?>" class="btn btn-secondary-outlined btn-half-width">
					<!-- <button type="button" class="btn"> -->
						<span class="hide-on-click"><?php echo _yani_theme()->get_option('spl_btn_call', 'Call'); ?></span>
				...
				<a target="_blank" href="https://api.whatsapp.com/send?phone=<?php echo esc_attr( $agent_whatsapp_call ); ?>&text=<?php echo _yani_theme()->get_option('spl_con_interested', "Hello, I am interested in").' ['.get_the_title().'] '.get_permalink(); ?> " class="btn btn-secondary-outlined <?php echo esc_attr($whatsappBtnClass); ?>"><i class="yani-icon icon-messaging-whatsapp mr-1"></i> <?php esc_html_e('WhatsApp', _yani_theme()->get_text_domain()); ?></a>
				<?php } ?>
