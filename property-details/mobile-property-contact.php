...
<div class="mobile-property-contact visible-on-mobile">
	<div class="d-flex justify-content-between">
		...
		<button class="btn btn-secondary" data-toggle="modal" data-target="#mobile-property-form">
			<i class="yani-icon icon-messages-bubble"></i>
		</button>

		<?php if( !empty( $agent_whatsapp_call ) && yani_option('agent_whatsapp_num', 1) ) { ?>
		<a href="https://api.whatsapp.com/send?phone=<?php echo esc_attr( $agent_whatsapp_call ); ?>&text=<?php echo yani_option('spl_con_interested', "Hello, I am interested in").' ['.get_the_title().'] '.get_permalink(); ?> " class="btn btn-secondary-outlined"><i class="yani-icon icon-messaging-whatsapp mr-1"></i></a>
		<?php } ?>
		
		<?php if( ! empty($agent_number_call) && yani_option('agent_mobile_num', 1) ) { ?>
		<a class="btn btn-secondary-outlined" href="tel:<?php echo esc_attr($agent_number_call); ?>">
	         <i class="yani-icon icon-phone"></i>
	     </a>
	 	<?php } ?>
