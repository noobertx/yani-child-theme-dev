...
$yani_yelp_api_key = yani_option('yani_yelp_api_key');
...
$yelp_data = yani_option( 'yani_yelp_term' );
$yelp_dist_unit = yani_option( 'yani_yelp_dist_unit' );
$prop_location = get_post_meta( get_the_ID(), 'yani_property_location', true );
...
<div class="what-nearby">
	<?php
    $link = site_url('wp-admin/admin.php?page=yani_options&tab=30');
    if( empty( $yani_yelp_api_key ) ) {
        echo '<div class="yelp-cat-block">';
        echo esc_html__('Please supply your API key', _yani_theme()->get_text_domain()).' ';
        echo '<a target="_blank" href="'.$link.'">'.esc_html__('Click Here', _yani_theme()->get_text_domain()).'</a>';
        echo '</div>';
    } else {
        ...
        foreach ( $yelp_data as $value ) :
            ...
            $response = yani_yelp_query_api( $term_id, $prop_location );
    					...
    					<div class="what-nearby-right">
    						<div class="rating-wrap">
    							<div class="rating-container">
    								<div class="rating">                                            
    									<?php echo yani_get_stars($data->rating); ?>

    									<span class="time-review"><?php echo $data->review_count; ?> <?php esc_html_e('reviews', _yani_theme()->get_text_domain());?></span>
    								</div>
    							</div>
    						</div>
    					</div>
