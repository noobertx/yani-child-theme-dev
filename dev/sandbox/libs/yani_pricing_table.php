 ...
 * Since V1.4.0
 */
if( !function_exists('yani_price_table') ) {
    function yani_price_table($atts, $content = null)
    {
        ...
        $package_custom_link    = get_post_meta( $package_id, 'yani_package_custom_link', true );
        ...
        if( $package_data == 'dynamic' ) {
            ...
            $pack_price              = get_post_meta( $package_id, 'yani_package_price', true );
            ...
            $pack_billing_frquency   = get_post_meta( $package_id, 'yani_billing_unit', true );
            $pack_package_tax   = get_post_meta( $package_id, 'yani_package_tax', true );
            $yani_package_popular    = get_post_meta( $package_id, 'yani_package_popular', true );
            ...
            <div class="price-table-description">
                <ul class="list-unstyled">
                    <li>
                        ...
                        <?php echo $yani_local['time_period'].':'; ?> <strong><?php echo esc_attr( $pack_billing_frquency ).' '._yani_theme()->get_billing_period( $pack_billing_period ); ?></strong>
                    </li>
                    <li>
                        <i class="yani-icon icon-check-circle-1 primary-text mr-1"></i>
                        <?php echo $yani_local['properties'].':'; ?> 
                    ...
                    <li>
                        <i class="yani-icon icon-check-circle-1 primary-text mr-1"></i>
                        <?php echo $yani_local['featured_listings'].':'; ?> <strong><?php echo esc_attr( $pack_featured_listings ); ?></strong>
                    </li>
                    <?php if( !empty($pack_package_tax)) { ?>
                        <li><i class="yani-icon icon-check-circle-1 primary-text mr-1"></i>
                            <?php echo $yani_local['package_taxes'].':'; ?> <strong><?php echo esc_attr( $pack_package_tax ); ?></strong></li>
                    <?php } ?>
                </ul>
            </div><!-- price-table-description -->
            <div class="price-table-button">
                <?php if( _yani_theme()->is_woocommerce() ) { ?>
                    <a class="houzez-woocommerce-package btn btn-primary" data-packid="<?php echo intval($package_id); ?>" href="#"><?php echo esc_attr( $yani_local['get_started'] ); ?></a>
                <?php } else { ?>
                    <a class="btn btn-primary" href="<?php echo esc_url($payment_process_link); ?>"><?php echo esc_attr( $yani_local['get_started'] ); ?></a>
                <?php } ?>
    ...
    add_shortcode('yani_price_table', 'yani_price_table');
}