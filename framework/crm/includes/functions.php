...
if( !function_exists('yani_crm_get_form_user_type') ) {
    function yani_crm_get_form_user_type($token) {
    
       $value = '';
       
       if( $token == 'buyer' ) {
            $value = yani_option('spl_con_buyer', "I'm a buyer");

       } else if( $token == 'tennant' ) {
            $value = yani_option('spl_con_tennant', "I'm a tennant");

       } else if( $token == 'agent' ) {
            $value = yani_option('spl_con_agent', "I'm an agent");

       } else if( $token == 'other' ) {
            $value = yani_option('spl_con_other', "Other");
       } else {
            $value = $token;
       }
...
if( !function_exists('matched_listings')) {
    function matched_listings($meta) {
        ...
        // Taxonomies
        $tax_query = apply_filters( 'yani_tax_crm_filter', $tax_query, $meta );
        ...
        // Meta
        $meta_query = apply_filters( 'yani_meta_crm_filter', $meta_query, $meta );
...
if(!function_exists('yani_matched_prop_type')) {
    function yani_matched_prop_type($tax_query, $meta) {
    ...
    add_filter('yani_tax_crm_filter', 'yani_matched_prop_type', 10, 2);
}


if(!function_exists('yani_matched_country')) {
    function yani_matched_country($tax_query, $meta) {
    ...
    add_filter('yani_tax_crm_filter', 'yani_matched_country', 10, 2);
}

if(!function_exists('yani_matched_state')) {
    function yani_matched_state($tax_query, $meta) {
    ...
    add_filter('yani_tax_crm_filter', 'yani_matched_state', 10, 2);
}

if(!function_exists('yani_matched_city')) {
    function yani_matched_city($tax_query, $meta) {
    ...
    add_filter('yani_tax_crm_filter', 'yani_matched_city', 10, 2);
}

if(!function_exists('yani_matched_area')) {
    function yani_matched_area($tax_query, $meta) {
    ...
    add_filter('yani_tax_crm_filter', 'yani_matched_area', 10, 2);
}

if(!function_exists('yani_matched_price')) {
    function yani_matched_price($meta_query, $meta) {
        if (isset($meta['min_price']) && !empty($meta['min_price']) && isset($meta['max_price']) && !empty($meta['max_price'])) {
            $min_price = doubleval(yani_clean($meta['min_price']));
            $max_price = doubleval(yani_clean($meta['max_price']));

            if ($min_price > 0 && $max_price >= $min_price) { 
                $meta_query[] = array(
                    'key' => 'yani_property_price',
    ...
    add_filter('yani_meta_crm_filter', 'yani_matched_price', 10, 2);
}

if(!function_exists('yani_matched_bedrooms')) {
    function yani_matched_bedrooms($meta_query, $meta) {
        if (isset($meta['min_beds']) && !empty($meta['min_beds']) && isset($meta['max_beds']) && !empty($meta['max_beds'])) {
            $min_beds = doubleval(yani_clean($meta['min_beds']));
            $max_beds = doubleval(yani_clean($meta['max_beds']));

            if ($min_beds > 0 && $max_beds >= $min_beds) { 
                $meta_query[] = array(
                    'key' => 'yani_property_bedrooms',
    ...
    add_filter('yani_meta_crm_filter', 'yani_matched_bedrooms', 10, 2);
}

if(!function_exists('yani_matched_bathrooms')) {
    function yani_matched_bathrooms($meta_query, $meta) {
        if (isset($meta['min_baths']) && !empty($meta['min_baths']) && isset($meta['max_baths']) && !empty($meta['max_baths'])) {
            $min_baths = doubleval(yani_clean($meta['min_baths']));
            $max_baths = doubleval(yani_clean($meta['max_baths']));

            if ($min_baths > 0 && $max_baths >= $min_baths) { 
                $meta_query[] = array(
                    'key' => 'yani_property_bathrooms',
    ...
    add_filter('yani_meta_crm_filter', 'yani_matched_bathrooms', 10, 2);
}

if(!function_exists('yani_matched_area_size')) {
    function yani_matched_area_size($meta_query, $meta) {
        if (isset($meta['min_area']) && !empty($meta['min_area']) && isset($meta['max_area']) && !empty($meta['max_area'])) {
            $min_area = doubleval(yani_clean($meta['min_area']));
            $max_area = doubleval(yani_clean($meta['max_area']));

            if ($min_area > 0 && $max_area >= $min_area) { 
                $meta_query[] = array(
                    'key' => 'yani_property_size',
    ...
    add_filter('yani_meta_crm_filter', 'yani_matched_area_size', 10, 2);
}

if(!function_exists('yani_mathed_zipcode')) {
    function yani_mathed_zipcode($meta_query, $meta) {
    ...
    add_filter('yani_meta_crm_filter', 'yani_mathed_zipcode', 10, 2);
}
...
if( !function_exists('houzezcrm_get_assigned_agent')) {
	function houzezcrm_get_assigned_agent( $agent_id, $type = "" ) {
		...
		if( $type == 'author_info' ) {
			...
			$data['email'] = get_post_meta($agent_id, 'yani_agency_email', true);

		} else {
			$data['name'] = get_the_title($agent_id);
			$data['email'] = get_post_meta($agent_id, 'yani_agent_email', true);
		}
		return $data;
	}
}
