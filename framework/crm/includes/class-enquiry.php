...
if ( ! class_exists( 'yani_Enquiry' ) ) {
	...
	class yani_Enquiry {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_yani_delete_enquiry', array( $this, 'delete_enquiry') );
			add_action( 'wp_ajax_yani_match_listing_email', array( $this, 'send_match_listing_email') );
		...
		public function save_enquiry($lead_id = "") {
			...
			if( (isset($_POST['yani_contact_form']) && $_POST['yani_contact_form'] == 'yes') || (isset($_POST['is_estimation']) && $_POST['is_estimation'] == 'yes') || empty($user_id) ) {
            ...
            $data_table        = $wpdb->prefix . 'yani_crm_enquiries';
	        $data = array(
	        	'user_id'       	=> $user_id,
                'lead_id'           => $lead_id,
		...
		public function get_property_info($listing_id) {
			...
			$enquiry_meta['min_beds'] = get_post_meta( $listing_id, 'yani_property_bedrooms', true );
			$enquiry_meta['max_beds'] = get_post_meta( $listing_id, 'yani_property_bedrooms', true );

			$enquiry_meta['min_baths'] = get_post_meta( $listing_id, 'yani_property_bathrooms', true );
			$enquiry_meta['max_baths'] = get_post_meta( $listing_id, 'yani_property_bathrooms', true );
			$enquiry_meta['min_price'] = get_post_meta( $listing_id, 'yani_property_price', true );
			$enquiry_meta['max_price'] = get_post_meta( $listing_id, 'yani_property_price', true );

			$enquiry_meta['min_area'] = get_post_meta( $listing_id, 'yani_property_size', true );
			$enquiry_meta['max_area'] = get_post_meta( $listing_id, 'yani_property_size', true );
			$enquiry_meta['zipcode'] = get_post_meta( $listing_id, 'yani_property_zip', true );
