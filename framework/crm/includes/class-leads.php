<?php
if ( ! class_exists( 'yani_Leads' ) ) {

	class yani_Leads {

		
		public function __construct() {
			add_action( 'wp_ajax_yani_crm_add_lead', array( $this, 'add_lead' ) );
			add_action( 'wp_ajax_get_single_lead', array( $this, 'get_single_lead' ) );
			add_action( 'wp_ajax_yani_delete_lead', array( $this, 'delete_lead') );
		}
		...
		public function save_lead() {
			...
			if ( isset( $_POST['user_type'] ) ) {
				...
				$user_type = yani_crm_get_form_user_type($user_type);
			}
			...
			if(isset($_POST['realtor_page']) && $_POST['realtor_page'] == 'yes') {
				if($agent_type == 'author_info') {
					...
					$user_id = get_post_meta( $agent_id, 'yani_user_meta_id', true );
				}
			} 
            ...
            $leads_table        = $wpdb->prefix . 'yani_crm_leads';
	        $data = array(
	        	'user_id'       => $user_id,
                'prefix'        => $prefix,
