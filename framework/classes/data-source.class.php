	...
	class yani_Data_Source{
		...
		static function shortcode_to_args($atts = '', $paged = '') {
            ...
            if( !empty($yani_user_role) ) {
                $role_ids = self::get_author_ids_by_role( $yani_user_role );
                if (!empty($role_ids)) {
            ...
            if (!empty($property_type) && $sort_taxonomy != 'property_type') {
                $tax_query[] = array(
                    ...
                    'field' => 'slug',
                    'terms' => self::traverse_comma_string($property_type)
                );
            }
            if (!empty($property_status) && $sort_taxonomy != 'property_status') {
                $tax_query[] = array(
                    ...
                    'field' => 'slug',
                    'terms' => self::traverse_comma_string($property_status)
                );
            }
            if (!empty($property_state)) {
                $tax_query[] = array(
                    ...
                    'field' => 'slug',
                    'terms' => self::traverse_comma_string($property_state)
                );
            }
            ...
            if (!empty($property_country)) {
                $tax_query[] = array(
                    ...
                    'field' => 'slug',
                    'terms' => self::traverse_comma_string($property_country)
                );
            }
            if (!empty($property_city) && $sort_taxonomy != 'property_city' ) {
                $tax_query[] = array(
                    ...
                    'field' => 'slug',
                    'terms' => self::traverse_comma_string($property_city)
                );
            }
            if (!empty($property_area)) {
                $tax_query[] = array(
                    ...
                    'field' => 'slug',
                    'terms' => self::traverse_comma_string($property_area)
                );
            }
            if (!empty($property_label)) {
                $tax_query[] = array(
                    ...
                    'field' => 'slug',
                    'terms' => self::traverse_comma_string($property_label)
                );
            }
            ...
            if (!empty($min_price) && !empty($max_price)) {
                $min_price = doubleval(yani_clean($min_price));
                $max_price = doubleval(yani_clean($max_price));
            ...
            $wp_query_args['posts_per_page'] = 3;
		...
		static function &get_wp_query ($atts = '', $paged = '') {
            ...
            $yani_query = new WP_Query($args);
            return $yani_query;
        }

        public static function get_author_ids_by_role($role) {
