<?php
/**
 * Noonie customizer base additional control class for theme customize options.
 *
 * Class Noonie_Customize_Base_Additional_Control
 *
 * @package    ThemeGrill
 * @subpackage Noonie
 * @since      Noonie 3.0.0
 ...
 * Class Noonie_Customize_Base_Additional_Control
 */
class Noonie_Customize_Base_Additional_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_style(
			...
			array(),
			NOONIE_THEME_VERSION
		);

		// Main CSS file.
		wp_enqueue_style(
			'noonie-customize-controls',
			$this->get_assets_url() . '/assets/css/customize-controls' . $suffix . '.css',
			array( 'wp-color-picker' ),
			NOONIE_THEME_VERSION
		);
		wp_style_add_data( 'noonie-customize-controls', 'rtl', 'replace' );
