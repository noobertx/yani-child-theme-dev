...
if( !class_exists('yani_Insights')) {
    $msg = esc_html__('Please install and activate Favethemes Insights plugin.', _yani_theme()->get_text_domain());
    wp_die($msg);
}

$user_id = get_current_user_id();
$insights = new yani_Insights();

$listing_id = isset($_GET['listing_id']) ? $_GET['listing_id'] : '';

if(!empty($listing_id)) {
    $insights_stats = $insights->yani_listing_stats($_GET['listing_id']);
} else {
    $insights_stats = $insights->yani_user_stats($user_id);
}
?>
    ...
    <div class="dashboard-header-wrap">
        <div class="d-flex align-items-center">
            <div class="dashboard-header-left flex-grow-1">
                <h1><?php echo yani_option('dsh_insight', 'Insight'); ?></h1>         
