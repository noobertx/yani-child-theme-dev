...
$dash_properties_link = _yani_template()->get_template_link('template/user_dashboard_properties.php');
...
if( $enable_paid_submission == 'per_listing' || $enable_paid_submission == 'free_paid_listing' ) {
    ...
    $clientSecret           =   _yani_theme()->get_option('paypal_client_secret_key');
    ...
    if ( isset($_GET['token']) && isset($_GET['PayerID']) ){
        ...
        /* Get saved data in database during execution
         -----------------------------------------------*/
        $transfered_data     = get_option('yani_paypal_transfer');
        ...
        $json_response  = _yani_payment()->execute_paypal_request( $payment_execute_url, $json, $token );

        $transfered_data[$current_user->ID ]  =   array();
        update_option ('yani_paypal_transfer',$transfered_data);
        ...
        if( $json_response['state']=='approved' ) {
            ...
            if( $is_prop_upgrade == 1 ) {
                ...
                $invoiceID = _yani_invoice()->generate_invoice( 'Upgrade to Featured','one_time', $prop_id, $date, $userID, 0, 1, '', $paymentMethod );
                update_post_meta( $invoiceID, 'invoice_payment_status', 1 );
                update_post_meta( $prop_id, 'yani_featured', 1 );
                //yani_email_to_admin('email_upgrade');
                ...
                update_post_meta( $prop_id, 'yani_payment_status', 'paid' );
                ...
                if( $is_prop_featured == 1 ) {
                    update_post_meta( $prop_id, 'yani_featured', 1 );
                    $invoiceID = _yani_invoice()->generate_invoice( 'Listing with Featured','one_time', $prop_id, $date, $userID, 1, 0, '', $paymentMethod );
                } else {
                    $invoiceID = _yani_invoice()->generate_invoice( 'Listing','one_time', $prop_id, $date, $userID, 0, 0, '', $paymentMethod );
                }
    ...
    if (isset($_GET['token'])) {
        ...
        // get transfer data
        $save_data = get_option('yani_paypal_package_transfer');
        ...
        if ($recursive != 1) {
            if (isset($_GET['PayerID'])) {
                ...
                $json = json_encode($payment_execute);
                $json_resp = _yani_payment()->execute_paypal_request($payment_execute_url, $json, $token);

                $save_data[$current_user->ID] = array();
                update_option('yani_paypal_package_transfer', $save_data);
                update_user_meta($userID, 'yani_paypal_package', $save_data);
         ...
         //end recursive if condition
            ...
            if($json_resp['state']=='Active' && $json_resp['payer']['status'] == 'verified' ) {
                ...
                delete_post_meta($pack_id, 'yani_paypal_billing_plan_'.$is_paypal_live);

                $invoiceID = _yani_invoice()->generate_invoice( 'package', 'recurring', $pack_id, $date, $userID, 0, 0, '', $paymentMethod, 1 );
                update_post_meta( $invoiceID, 'invoice_payment_status', 1 );

                update_user_meta( $userID, 'yani_paypal_recurring_profile_id', $profileID );
            ...
            <div class="col-12">
                <div class="dashboard-content-block">
                    ...
                    if( isset( $_GET['directy_pay'] ) && $_GET['directy_pay'] != '' ) {
                        ...
                        $invoice_meta = _yani_invoice()->get_invoice_meta( $orderID );
                        ?>
                        <p><strong><?php echo _yani_theme()->get_option('thankyou_wire_title'); ?></strong></p>
                        <ul style="text-align: left;">
                            <li><?php echo $yani_local['order_number'].':'; ?> <strong><?php echo esc_attr($orderID); ?></strong> </li>
                            <li><?php echo $yani_local['date'].':'; ?> <strong><?php echo get_the_date('', $orderID); ?></strong> </li>
                            <li><?php echo $yani_local['total'].':'; ?> <strong><?php echo yani_get_invoice_price( $invoice_meta['invoice_item_price'] );?></strong> </li>
                            <li><?php echo $yani_local['payment_method'].':'; ?>
                                <strong>
                        ...
                        <p> <?php echo _yani_theme()->get_option('thankyou_wire_des'); ?></p>

                    <?php
                    } else { ?>

                    <p><strong><?php echo _yani_theme()->get_option('thankyou_title'); ?></strong></p>
                    <p><?php echo _yani_theme()->get_option('thankyou_des'); ?></p>
                    <?php } ?>
                    <a class="btn btn-primary-outlined" href="<?php echo esc_url( $dash_properties_link ); ?>"><?php echo $yani_local['goto_dash']; ?></a>
