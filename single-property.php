...
$single_top_area = get_post_meta( get_the_ID(), 'yani_single_top_area', true );
$single_content_area = get_post_meta( get_the_ID(), 'yani_single_content_area', true );
$map_street_view = get_post_meta( get_the_ID(), 'yani_property_map_street_view', true );
$loggedin_to_view = get_post_meta( get_the_ID(), 'yani_loggedintoview', true );
$property_live_status = get_post_status();
$hide_fields = _yani_theme()->get_option('hide_detail_prop_fields');
_yani_post()->count_property_views( $post->ID );

$enable_disclaimer = _yani_theme()->get_option('enable_disclaimer', 1);
$global_disclaimer = _yani_theme()->get_option('property_disclaimer');
$property_disclaimer = get_post_meta( get_the_ID(), 'yani_property_disclaimer', true );
...
if( $sticky_sidebar['single_property'] != 0 ) { 
    $is_sticky = 'yani_sticky'; 
}

$is_full_width = _yani_theme()->get_option('is_full_width');
$top_area = _yani_theme()->get_option('prop-top-area');
$property_layout = _yani_theme()->get_option('prop-content-layout');
...
if( _yani_theme()->is_elementor() && ( ! function_exists( 'elementor_theme_do_location' ) || ! elementor_theme_do_location( 'single' ) ) ) {
                ...
                <div class="row">
                    <div class="<?php echo esc_attr($content_classes); ?>">
                        ...
                        <div class="property-view">
                            ...
                            if( _yani_theme()->get_option('enable_next_prev_prop') ) {
